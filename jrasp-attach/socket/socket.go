...
func (this *SocketClient) Handler(p *Package) {
	...
	case UPDATE:
		s := string(p.Body)
		log.Println("update parameters result:" + s)
	default:
		log.Printf("result:%s", string(p.Body))
	}
}
...
func (this *SocketClient) UnloadModule(message string) {
	this.Send(message, UNLOAD)
}
