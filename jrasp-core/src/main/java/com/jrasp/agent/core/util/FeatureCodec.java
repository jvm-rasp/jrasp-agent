...
public class FeatureCodec {
    ...
    public String toString(final Collection<String> collection) {
        ...
        for (String value : collection) {
            if (RaspStringUtils.isNotBlank(value)) {
                featureSB.append(escapeEncode(value)).append(kvSegmentSeparator);
            }
        }
        return featureSB.toString();
    }
    ...
    public Collection<String> toCollection(final String featureString) {
        ...
        if (RaspStringUtils.isBlank(featureString)) {
            return collection;
        }
        for (String value : escapeSplit(featureString, kvSegmentSeparator)) {
            if (RaspStringUtils.isBlank(value)) {
                // 过滤掉为空的字符串片段
                continue;
            }
    ...
    public String toString(final Map<String, String> map) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            if (RaspStringUtils.isNotBlank(key) && RaspStringUtils.isNotBlank(value)) {
                featureSB.append(escapeEncode(key)).append(kvSeparator).append(escapeEncode(value)).append(kvSegmentSeparator);
            }
        }
        return featureSB.toString();
    }
    ...
    public Map<String, String> toMap(final String featureString) {
        ...
        for (String kv : escapeSplit(featureString, kvSegmentSeparator)) {
            ...
            final String k = ar[0];
            final String v = ar[1];
            if (RaspStringUtils.isNotBlank(k) && RaspStringUtils.isNotBlank(v)) {
    ...
    private boolean isSpecialChar(final char c) {
        return c == kvSegmentSeparator || c == kvSeparator || c == ESCAPE_PREFIX_CHAR;
    }
    ...
    private void addToArrayList(final ArrayList<String> segmentArrayList, final String segmentString) {
        ...
        segmentArrayList.add(segmentString.trim());
    }
